...
def draw(file_name, n, period):  
    ...
    # create jpg -> mp4 
    DIR = output_path+"/process"
    #check number of jpg files
    file_count = (len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))]))
    target_path = output_path+"/process" + '/%04d.jpg'
    CLIP_FPS = 20.0
    ...
    output_mp4_file = './html/mp4/' + 'result.mp4'
    video = cv2.VideoWriter(output_mp4_file,fourcc, CLIP_FPS, vid_width_height)
    for i in range(1, file_count):
        img = cv2.imread(target_path % i)
        if img is None:
            print("can't read")
            break
        # add
    ...
    video.release()
    print('The mp4 file has been generated successfully.')

    # cleanup files
    result_file_name = './html/img/' + "result.jpg"
    input_file_name = './html/img/' + "input.jpg"
    shutil.move(output_path + "/result_RGB.jpg", result_file_name)
    shutil.move(output_path + "/input.jpg", input_file_name)
